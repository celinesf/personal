...
'data4estlik' <- function(paramfile="estimation.par",datafile="datafiles/reg", regfile="datafiles/info.reg", locifile="datafiles/info.loc", summaries="S_1 S_2 S_sl S_sh F_st D_1 D_2 D_star1 D_star2",type=c(0,1,5,10),nreg=1)
{
  ok=1
    liststats=list()
                                ...
                                if(check$ok)
                                  {
                                    ...
                                    data[[nr]]$cmdline_seeds[1]=cmd[1]
                                     print(c("bfore getstat ",nr))
                                    st=stats_pop(msframe= data[[nr]], type=type)
                                    print(c("after getstat ",nr))
