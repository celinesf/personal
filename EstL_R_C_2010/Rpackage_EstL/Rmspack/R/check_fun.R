...
'check_param' <-
  function(param,paramfile, listparam, type, summaries)
{
  ret=list()
  ret$ok=1;
  ret$param=param  
...
### list of stats
		...
		{
		if(is.na(summaries)){
        tp=c("WARNING: The list of statistics has the value ",summaries," when I expect a list of summary statistcis to consider. I am therefore going to use the default list ",ret$summaries,"."); stp=c("'","'","'","'",""); error_message(tp,stp); 
        	}else
