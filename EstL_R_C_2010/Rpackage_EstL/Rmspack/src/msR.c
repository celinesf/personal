...
/*! \file msR.c
  \brief  Generates samples of gametes ( theta given or fixed number 
...
SEXP msMain(SEXP cmd , Rboolean onexit)
{
  int i, k, howmany, segsites ;
  char **list,**tbsparamstrs ;
  double probss, tmrca, ttot ;

  /* added to pick command line from R */
  int  argc;
  char *argv[100], *cmdline[1], *token,  tpcmd[1000]; //, tpargv[500];
  int get_arc(char *pcmd,char *delim );
 
  /*** argument = command line***/ 
  PROTECT(cmd = AS_CHARACTER(cmd)); 
  cmdline[0] = R_alloc(strlen(CHAR(STRING_ELT(cmd, 0))), sizeof(char)); 
  sprintf(cmdline[0], CHAR(STRING_ELT(cmd, 0))); 

  strcpy(tpcmd,cmdline[0]); /* copy cmd */
  argc= get_arc(tpcmd, " " ); /* get narc ! tpcmd become ./ms */ 
  strcpy(tpcmd,cmdline[0]); /* copy cmd */
  token = strtok(tpcmd , " ");  /* get argv ! tpcmd become ./ms */ 
  argv[0]=token;
  for(i=1;i<argc;i++)
    {
      token = strtok( NULL, " " );
  ...
  /** Declaration of output objects in R **/
  SEXP list_names, haplo, myseg,mypos,v_mypos, mylist, mycl, myseed;  
   
  int *p_myseg; 
  double *p_mypos;
  char **p_mylist;
  char *names[4] = {"cmdline_seeds", "segsites", "positions", "haplotypes"};// name for various object 

  PROTECT(mycl =allocVector(STRSXP,2)); 
  SET_STRING_ELT(mycl,0,mkChar(cmdline[0]));  /* command line */
  sprintf(token,"%d %d %d",pars.tableseeds[0],pars.tableseeds[1],pars.tableseeds[2]);
  SET_STRING_ELT(mycl,1,mkChar(token));  /* command line */

  PROTECT(list_names = allocVector(STRSXP,4));   
  for(i = 0; i < 4; i++)   
    SET_STRING_ELT(list_names,i,mkChar(names[i]));
  PROTECT(haplo = allocVector(VECSXP, 4)); // protect haplo R object
  PROTECT(myseg = allocMatrix(INTSXP,howmany,1));  // protect seg site R object
  p_myseg = INTEGER_POINTER(myseg);
  PROTECT(mypos =  allocVector(VECSXP,howmany));
  PROTECT(mylist =  allocMatrix(STRSXP, howmany,pars.cp.nsam)); // protect list of haplo R object   
  /****/
  ...
  while( howmany-count++ ) 
    {      
      ...
      /* added for R */
      p_myseg[count-1]=segsites;/* added for R */
      v_mypos= NEW_NUMERIC(segsites);// Dynamic vector- change foe each independent sample
      p_mypos = NUMERIC_POINTER(v_mypos);
      /****/
      if( pars.mp.timeflag ) Rprintf("time:\t%lf\t%lf\n",tmrca, ttot ) ;
      if( (segsites > 0 ) || ( pars.mp.theta > 0.0 ) ) 
        {
          for( i=0; i<segsites; i++)
            p_mypos[i]=posit[i];               /* added for R */
          SET_VECTOR_ELT(mypos,count-1,v_mypos); /* added for R */
          if( segsites > 0 ) 
            for(i=0;i<pars.cp.nsam; i++)
              SET_STRING_ELT(mylist,count-1+howmany*i,mkChar(list[i]));/* added for R */
        }
    }// end of loop on samples
  if( !pars.commandlineseedflag ) seedit( "end",&pars );/* change celine */
 
  /* attaching the vector names for R list obect */
  SET_VECTOR_ELT(haplo, 0, mycl);
  SET_VECTOR_ELT(haplo, 1, myseg);
  SET_VECTOR_ELT(haplo, 2, mypos); 
  SET_VECTOR_ELT(haplo, 3, mylist); 
  setAttrib(haplo, R_NamesSymbol, list_names); 
  UNPROTECT(7);
  ...
  /*****/
  return(haplo); /* list return to R*/
  /*****/
}// end main R

int get_arc(char *pcmd,char *delim )
{
  char *token = strtok(pcmd,delim);
  int numToks = 0;
  while ( token != NULL ){
    numToks++;   
            ...
            char ** list )
{
  ...
  for( i=0; i<nsam; i++){
    ...
    if( list[i] == NULL ) error("I didn't finish running ms bigger list"); //perror( "realloc error. bigger");
  }
  return(0);
}
...
usage()
{
  fprintf(stderr,"usage: ./msR nsam howmany \n");
