...
/*     printf("\n"); */
  ...
  if( !pars.commandlineseedflag ) seedit( "s", &pars); /* change celine */// for write seeds in file //  printf("\n%d %d %d\n",pars.tableseeds[0],pars.tableseeds[1],pars.tableseeds[2]);// print seeds // comment
  if( pars.tp.pf)
...
{
  int nr,  sstat=0;
  char *dum=NULL, tp[10], line[1000], tpl[1000];
  ...
  if(pars.sp.nsp>0)
    {
      ...
      if(!( pars.sp.cov= (double **)calloc( (unsigned) pars.sp.nsp,sizeof( double *))))
        perror(" calloc error. cov"); // covariance matrix
      if(!( pars.sp.icov= (double **)calloc( (unsigned) pars.sp.nsp,sizeof( double *))))
      ...
      if(!(pars.sp.mstat= (double **)calloc( (unsigned) pars.sp.nsp,sizeof( double *))))
      ...
      for(i=0;i<pars.sp.nsp;i++)
        {
          ...
          if(!(pars.sp.cov[i]= (double *)calloc( (unsigned) pars.sp.nsp,sizeof( double))))
            perror(" calloc error. covi");
          if(!(pars.sp.icov[i]= (double *)calloc( (unsigned) pars.sp.nsp,sizeof( double))))
            perror(" calloc error. icovi");
          if(!(pars.sp.ncov[i]= (int *)calloc( (unsigned) pars.sp.nsp,sizeof( int))))
            perror(" calloc error. ncov");
          if(!(pars.sp.mstat[i]= (double *)calloc( (unsigned) 2,sizeof( double) )))
          ...
          if(pars.sp.pstat[i][1]==1 && pars.sp.pstat[j][1]==1)
            {
              ...
              pars.sp.cov[i][j]+= pars.sp.pstat[i][0]* pars.sp.pstat[j][0];
              pars.sp.ncov[i][j]++;
              pars.sp.ncov[j][i]= pars.sp.ncov[i][j];
              pars.sp.cov[j][i]= pars.sp.cov[i][j];
...
long double get_tot_lik(  clock_t pstartl, int nregarg, int init)
{
  ...
  if(!( cov= (double **)calloc( (unsigned) pars.sp.nsp,sizeof( double *))))
    perror(" calloc error. cov"); // covariance matrix
  for(i=0;i<pars.sp.nsp;i++)
    {
      if(!(cov[i]= (double *)calloc( (unsigned) pars.sp.nsp,sizeof( double))))
        perror(" calloc error. covi");
    }
          ...
          for (j=0; j <=i ; j++)
            {
              if(pars.sp.ncov[i][j]>1)
                {
                  //printf("BEF i%d j%d c%lg \t %lg %lg\t%lg\n", i, j,pars.sp.cov[i][j],(double) pars.sp.cov[i][j]/pars.sp.ncov[i][j] ,((double) pars.sp.pstat[i][5+j*2+1]/ pars.sp.pstat[i][5+j*2]) *((double) pars.sp.pstat[j][5+i*2+1]/pars.sp.pstat[j][5+i*2]),(double) pars.sp.ncov[i][j]/(pars.sp.ncov[i][j]-1)*((double) pars.sp.cov[i][j]/pars.sp.ncov[i][j] - ((double) pars.sp.pstat[i][5+j*2+1]/ pars.sp.pstat[i][5+j*2]) *((double) pars.sp.pstat[j][5+i*2+1]/pars.sp.pstat[j][5+i*2])));
                  sprintf(tp1,"%lf",(double) pars.sp.cov[i][j]/pars.sp.ncov[i][j]);
                  sprintf(tp2,"%lf",((double) pars.sp.pstat[i][5+j*2+1]/ pars.sp.pstat[i][5+j*2]) *((double) pars.sp.pstat[j][5+i*2+1]/pars.sp.pstat[j][5+i*2]));
                  if( strcasecmp(tp1,tp2)==0  ) cov[i][j]=0;
                  else
                    cov[i][j]= (double) pars.sp.ncov[i][j]/(pars.sp.ncov[i][j]-1)*((double) pars.sp.cov[i][j]/pars.sp.ncov[i][j] - ((double) pars.sp.pstat[i][5+j*2+1]/ pars.sp.pstat[i][5+j*2]) *((double) pars.sp.pstat[j][5+i*2+1]/pars.sp.pstat[j][5+i*2]));
                                 
                  if(j!=i)
                    cov[j][i]=  cov[i][j];
                  //printf("AFT %d %d n%d c%lg %lg %lg %lg %lg %d \t %lf\n", i, j, pars.sp.ncov[i][j],pars.sp.cov[i][j],pars.sp.pstat[i][5+j*2+1],pars.sp.pstat[i][5+j*2],pars.sp.pstat[j][5+i*2+1],pars.sp.pstat[j][5+i*2], (int) (strcasecmp(tp1,tp2) ), cov[i][j] );
                }
              else
                { 
                  print_error(5,i , j);
...
void Inverse(double **a,double **inv,int n )
{
  ...
  for ( i = 0;i < n;i++ )
    for ( j = 0;j < n;j++ )
      {
        inv[ i ][ j ] =(double) inv[ i ][ j ] / d;
        if(d<1e-15 && d>-1e-15) inv[ i ][ j ]=log(-1) ; //printf("i %d %d %lf %lf\n",i,j,inv[ i ][ j ] ,d);
      }
}
...
void CoFactor(double **a,int n,double **b)
{
  ...
  if( ! (c =(double**)calloc((unsigned)(n),sizeof(double *))))
    perror(" calloc error. c");
  for (i=0;i<n;i++)
    if( ! (c[i] = calloc((unsigned)(n),sizeof(double))))
  ...
  for (j=0;j<n;j++) 
    {
      for (i=0;i<n;i++) 
        { 
          ...
          b[i][j] =(double) pow(-1.0,i+j+2.0) * det;
        }
    }
  for (i=0;i<n;i++)
    free(c[i]);
...
*/
void Transpose(double **a,double **m,int n)
{
  int i,j;
  double tp=0;
  for (i=0;i<n;i++)
...
double Determinant(double **a,int n)
{
  ...
  if (n < 1) {}  /* Error */
  ...
  else 
    {
      ...
      for (j1=0;j1<n;j1++) 
        { 
          if( ! ( m = (double **)calloc((unsigned)(n),sizeof(double *))))
            perror(" calloc error. m");
          for (i=0;i<n;i++)
            if( ! (m[i] = (double *)calloc((unsigned)(n),sizeof(double))))
