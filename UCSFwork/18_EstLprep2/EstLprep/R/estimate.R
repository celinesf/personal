...
`input_EstL` <-
function(paramfile="paramfile",datafile="datafiles/reg", regfile="datafiles/regfile", locifile="datafiles/locifile", outparam=TRUE,outdata=TRUE)
{
            ...
            if(file.exists(regfile))
              {
                ...
                data4EstL(paramfile=paramfile,datafile=datafile, regfile=regfile, locifile= locifile, summaries=summaries, type=type,nr=nr) 
              }
            else
...
'data4EstL' <- function(paramfile="estimation.par",datafile="datafiles/reg", regfile="datafiles/info.reg", locifile="datafiles/info.loc", summaries="S_1 S_2 S_sl S_sh F_st D_1 D_2 D_star1 D_star2",type=c(0,1,5,10),nreg=1)
{
  ok=1
    liststats=list()
    ...
    nregions=dim(reg)[1]
    if(nreg!=nregions)
      {tp=c("PROBLEM: I found ",nregions, " regions/lines in the file ",regfile, " but in the file of parameters I found ",nreg," regions. I didn't output the datafiles."); stp=c("'","'","'","'","'","'"); error_message(tp,stp)
      }else
      {
        loci=0
