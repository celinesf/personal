...
#         print s
    ...
    def getKeyValue(self, line):
        ...
        if key :                   
            ...
            ''' add new key '''
            self.key_list = self.addToList(self.key_list, key, {"count" : 0, "value" : {}}, "count")
            self.current_bioset = self.addToList(self.current_bioset, key,{"count" : 0, "value" : {}}, "count")
...
#             self.current_bioset[key]["batch"] = self.addToList(self.current_bioset[key]["batch"], self.batchname, 0, None)
    ...
    def addNewKeyCombination(self):
        ...
        if len(self.key_combination)>0:
            ...
            if self.key_comb_list is None:
                self.key_comb_list = {}
                self.key_comb_list = self.addToList(self.key_comb_list, "0", {"keys":self.key_combination,"count" : 0},"count")
...
#                 self.key_comb_list["0"] = {"keys":self.key_combination,"count":0,"batch":{}}
                ...
                for comb in self.key_comb_list:
                    ...
                    if it == len(self.key_combination) and it == len(self.key_comb_list[comb]['keys']):
                        found = 1
                        self.key_comb_list = self.addToList(self.key_comb_list, comb, {"keys":self.key_combination,"count" : 0},"count")
#                         self.key_comb_list[comb]["batch"] = self.addToList(self.key_comb_list[comb]["batch"], self.batchname, 0, None)
                ...
                if found == 0:
                    ...
                    self.key_comb_list = self.addToList(self.key_comb_list, comb_num, {"keys":self.key_combination,"count" : 0},"count")
...
#             self.batchname = batch
        ''' '''
        self.batchname = "gwasbiosets_july2013_CB3.txt"
        ''' '''
        print self.batchname   
        logging.debug(' Function:  RawDataCleanUp main - bathc %s' % self.batchname ) 

        input_data = io.open("%s/%s" % (config.DATAPATH,self.batchname),'r',encoding='utf-8-sig')
        ...
        while len(line)>0:
            ...
            while self.partition in line and len(line)>0:
                ...
                if self.issue == True:
                    self.issue_list.append({"current":self.current_bioset, "previous":self.previous_batch})
                self.issue = False
                if len(self.current_bioset)>0:
                    self.previous_batch = self.current_bioset
                self.current_bioset ={}
        ...
        self.writeOutput("key_%s" %self.batchname,self.key_list)

        self.writeOutput("key_comb_%s" %self.batchname,self.key_comb_list)
        self.writeOutput('column',self.column_name_dict)
        self.writeOutput('pmid',self.pmid_list)
        
#         self.writeList("key_list_%s" %self.batchname,self.key_list)
