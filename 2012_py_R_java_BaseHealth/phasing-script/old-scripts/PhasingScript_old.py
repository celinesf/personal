...
MONGO = "mongodb://adm1n/admin"
CW = pymongo.Connection(MONGO, 25565);


def get_argv(argv):
    file_illu = 'no file name'
    if len(argv) >1:
        dir_illu =sys.argv[1]
...
def run_Beagle(dir_illu,trios,unrelated,dir_unphased,dir_phased, fphased):
    ...
    for chr_num in unphased_name:  
        cmdline = 'java -Xmx1000m -jar beagle.jar missing=? markers=%s/%s/%s%s out=%s/%s/%s unphased=%s/%s/%s' % (dir_illu,dir_unphased,marker, chr_num.split('_chr')[1], dir_illu,dir_phased, fphased,dir_illu,dir_unphased,chr_num)
