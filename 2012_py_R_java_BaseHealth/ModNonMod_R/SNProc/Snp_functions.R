...
AveOrCalc <- function()
{
	...
	cat("prevalence of Disease is " , Pd, "\n")
	print(Freq)
...
EthMapHap  <- function()
{
	...
	ethn    <- c("TAIWAN_CH", "EASTASIAN", "NA", "NATIVEAMERICAN", "NATIVE_AMERICAN", "NORTHERNHANCHINESE", "VARIOUS", "CHINESE_KOREAN", "HONGKONG_CH", "SOUTHASIAN", "JAPANESE_CHINESE_MALAY", "ASIANVARIOUS", "SOUTHEASTASIAN", "CHINESE", "HANCHINESE", "NA", "KOREAN", "SOUTHKOREAN", "JAPANESE", "CAUCASIAN", "HISPANIC", "AFRICANAMERICAN")
	hapmap_ <- c("A", "A", "A", "NA", "NA", "A", "A", "A", "A", "A", "A", "A", "A", "A"      , "A"         , "A" , "A"     , "A"          , "JPT"     , "CEU"      , "MEX"     , "ASW")
...
GenotypeCalc <- function(subres, EthHap, id, Did)
{
	...
	for (i in c(1:nrow(subres)) )
	{
		if (is.na(subres[i,]$FreqPop) && is.na(subres[i,]$FreqControl))
		{
			...
			if (pop_ == "A")
			{
				...
				if ( any(grep("CH", toupper(subres[i,]$NarrowEthnicity) ) == 1 ) )
				{
					freq3=NA
	...
	if (!is.null(Freq) & any(!is.na(Freq)) )
	{
		...
		SNP_frame <- data.frame(RiskFactorID=id, RiskFactorValue=c(0, 1, 2, 3), snpID=snp, Genotype=c(NA, Geno), freq=c(0,Freq), OR=c(1, OR), NewOR=c(1,NewOR), BroadEthnicity=subres$BroadEthnicity[1], NarrowEthnicity=subres$NarrowEthnicity[1], name=genename$Gene, A_G='G', CI=c(NA, subres$CI), PValue=c(NA, subres$Pvalue) )  
		PushToGenUIDb(SNP_frame, Did)
		
		snip <-data.frame(RiskFactorID=rep(id, 4), RiskFactorValue=c(0, 1, 2, 3), FactorValue=c(1, log(NewOR)), Min=rep(0,4)  , Max=rep(3,4)  , normal=rep(0,4), name=rep(genename$Gene,4), comment=c("NA", Geno), Ethn=rep(paste(subres$BroadEthnicity[1], subres$NarrowEthnicity[1], sep="_"),4)) 
		#id <- id + 1
...
AlleleCalc <- function(subres, EthHap, id, Did)
{
	...
	# first check to see if freqpop is there
	Hardy      <- HardyFreq(subres)
	
	if (nrow(hap) > 0 || !is.na(Hardy))
	{
		...
		if (any(is.na(freq) ) )
		{
			cat("\nThis probably means no FreqPOP and no HapMap ... Time to seek Freq Ctr\n")
		...
		if ( (RefAllele%in%c(hap$MajorAllele, hap$MinorAllele) ) & (RiskAllele%in%c(hap$MajorAllele, hap$MinorAllele) ) )
		{
			...
			if (!is.null(Freq) & any(!is.na(Freq)) )
			{
				...
				# capture it
				SNP_frame <- data.frame(RiskFactorID=id, RiskFactorValue=c(0, 1, 2, 3), snpID=snp, Genotype=c(NA, Geno), freq=c(0,Freq), OR=c(1, OR), NewOR=c(1,NewOR), BroadEthnicity=subres$BroadEthnicity, NarrowEthnicity=subres$NarrowEthnicity, name=genename$Gene, A_G='A', CI=NA, PValue=NA )  
				PushToGenUIDb(SNP_frame, Did)
...
HardyFreq <- function(subres)
{
	...
	{
		cat("No FreqPop Found... Going to HapMap\n")
		return(NA)
	}
}
...
HardyFreq_Ctr <- function(subres)
{
	...
	{
		cat("No Freq Found... giving up on Freqs\n")
		return(NA)
	}
}
...
AdjustOR_XY <- function(subres, hap, id, EthHap, gene )
{
	...
	if (hap$Chromosome == 'X' )
	{
		...
		if (any(is.na(subres$FreqPop) ))
		{
			...
			if (subres[1,]$dbSNP == "rs5945572" & (toupper(subres[1,]$BroadEthnicity) =="ASIAN" ) )
			{
				AlleleFreqs <- c("G:0.92953737", "A:0.07046263")
			}else if (subres[1,]$dbSNP == "rs5945326" & (toupper(subres[1,]$BroadEthnicity) =="ASIAN" ) )
			{
				AlleleFreqs <- c("A:0.67", "G:0.33")
			}else if (subres[1,]$dbSNP == 'rs6152')
			{
				AlleleFreqs <- c("G:0.722", "A:0.278")
			}else 
			{			
				pop_ <- EthHap$HapMap[EthHap$Ethnicity== toupper(subres[1,]$BroadEthnicity)]  
			...
			if (is.na(MajAllele) || is.na(MinAllele) )
				return()
