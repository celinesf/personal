...
ethn    <- c("CHINESE", "SOUTHKOREAN", "JAPANESE", "CAUCASIAN", "HISPANIC", "AFRICANAMERICAN")
hapmap_ <- c("CHB", "CHB", "JPT", "CEU", "MEX", "ASW")
...
qur    <- ('select PMID, dbSNP, BroadEthnicity, NarrowEthnicity, Allele_Genotype, ORValue, FreqPop, ModelType from SNPOR where (Genophen like \'%es\' and DiseaseID=7)
		order by dbSNP, BroadEthnicity')
snpqur <- ('select Distinct dbSNP from SNPOR where (Genophen like \'%es\' and DiseaseID=7)')
...
{
	qur2<- (paste('select GeneNextBio, dbSNP from SNPOR  where (dbSNP=', '\'', snp, '\'', ')', sep=""))
	genename <- unique(dbGetQuery(con, qur2))
	if (is.na(genename$GeneNextBio) )
	{
		genename$GeneNextBio <- "NA"
	}
	if (genename$GeneNextBio%in%snpnames)
	{
		genename$GeneNextBio <- paste(genename$GeneNextBio, snpnamecounter, sep="_")
		snpnamecounter <- snpnamecounter + 1
	}
	snpnames <- c(snpnames, genename$GeneNextBio )
	...
	genetics_gui <- rbind(genetics_gui, data.frame(RiskFactorID=unique(SNP_frame$RiskFactorID[SNP_frame$snpID==snp]) , 
						       name=genename$GeneNextBio         , Min=0  , Max=3  , normal=0) )
	
}

# for demo 
if (nrow(genetics_gui) > 2 )
{
	genetics_gui <- genetics_gui[nrow(genetics_gui):(nrow(genetics_gui)-1),]
	snpnames     <- snpnames[length(snpnames):(length(snpnames)-1)]
	snpunits     <- snpunits[length(snpunits):(length(snpunits)-1)]
}
