...
TCH=rbind(TCH,cbind(TCHOL$SEQN,as.numeric(as.character(TCHOL$LBXTC))));
#HDL
TCHOL<-read.csv ("hdl_d.csv", header=T);
THDL=cbind(TCHOL$SEQN,as.numeric(as.character(TCHOL$LBDHDD)));
TCHOL<-read.csv ("l13_b.csv", header=T);
THDL=rbind(THDL,cbind(TCHOL$SEQN,as.numeric(as.character(TCHOL$LBDHDL))));
TCHOL<-read.csv ("l13_c.csv", header=T);
THDL=rbind(THDL,cbind(TCHOL$SEQN,as.numeric(as.character(TCHOL$LBDHDD))));
TCHOL<-read.csv ("lab13_a.csv", header=T);
THDL=rbind(THDL,cbind(TCHOL$SEQN,as.numeric(as.character(TCHOL$LBDHDL))));
