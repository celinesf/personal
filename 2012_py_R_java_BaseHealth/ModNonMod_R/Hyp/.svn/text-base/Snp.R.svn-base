...
setwd("/home/pouria3/workspace/GenStat/trunk/Dep")
...
qur    <- ('select PMID, dbSNP, BroadEthnicity, NarrowEthnicity, Allele_Genotype, ORValue, FreqPop, ModelType from SNPOR where (Genophen like \'%es\' and DiseaseID=8)
		order by dbSNP, BroadEthnicity')
snpqur <- ('select Distinct dbSNP from SNPOR where (Genophen like \'%es\' and DiseaseID=8)')
			...
			if (nrow(hap) > 0 )
			{
				...
				if ( (RefAllele%in%c(hap$MajorAllele, hap$MinorAllele) ) & (RiskAllele%in%c(hap$MajorAllele, hap$MinorAllele) ) )
				{
					...
					if (toupper(mtype) == "DOMINANT")
					{
						...
						snip <-rbind(snip, data.frame(RiskFactorID=rep(id, 4), RiskFactorValue=c(0, 1, 2, 3), FactorValue=c(0, log(NewOR)), Min=rep(0,4)  , Max=rep(3,4)  , normal=rep(NA,4), name=rep(NA,4), comment=c("NA", Geno)) )
						#id <- id + 1
...
genetics_gui <- data.frame(RiskFactorID=c() ,  Min=c()  , Max=c()  , normal=c(), name=c(), comment=c())
snpnames <- c()
snpunits <- c()
snpnamecounter <- 2
for (snp in unique(SNP_frame$snpID) )
{
	qur2<- (paste('select Gene, dbSNP from SNPInfo  where (dbSNP=', '\'', snp, '\'', ')', sep=""))
	...
	snip$name[snip$RiskFactorID==unique(SNP_frame$RiskFactorID[SNP_frame$snpID==snp]) ] <- genename$Gene 
	
	#genetics_gui <- rbind(genetics_gui, data.frame(RiskFactorID=unique(SNP_frame$RiskFactorID[SNP_frame$snpID==snp]) , 
	#			Min=0  , Max=3  , normal=0, name=genename$Gene, comment=genename$Gene) )
