 ...
 ****************************** Changes in different verions
  ...
  *--- Changed Feb. 11st 2010: ---*
  % mimar_noanc.c: 
  - Assumes that the summary statistics of the polymorphism data 
    were calculated without known ancestral alleles. 
    So shared sites are when both alleles are present in both population samples.
  -----------------------------
      ...
      if((lpop1==0)||(lpop2==0))
        {
          if(((lpop1==nsam1)&&(lpop2a<=nsam2))||((lpop2==nsam2)&&(lpop1a<=nsam1)))
            ...
            {
              if((lpop2==0)&&(lpop1<nsam1)) pl[0]+=timd;//L1
              else pl[1]+=timd;//L2
            }
        }
      else
        { /* Celine changed 02/11/2010 */ // for data without ancestral data
          if(lpop1==nsam1)
            pl[1]+=timd;// L2
          else if(lpop2==nsam2)
            pl[0]+=timd;// L1
