...
struct params getpars(int argc, char *argv[], int *phowmany)
{
  ...
  */
  ...
  p.cp.fout="-";
  p.cp.sumout=2;
  p.mp.mu=p.mp.g=p.mp.theta=p.mp.thetafix=0;
  ...
  */
      ...
      switch (argv[arg][1])
        {
          ...
          case 'l' :                          // Multiple loci information
                    ...
                    for(i=0;i<*phowmany;i++)
                      {
                        ...
                        for(j=0;j<11;j++)     // Init array of locus specific stats 
            ...
            /* else
               ...
               for(j=0;j<11;j++)         // Init array of locus specific stats
               p.lp[i].H[j]=0;      // Init array of locus statistics stats
               }// End loop along loci
               }// End option -l
            *//////
            break;
            /* Uncommented for Celine's use */ /* for gof */
            /*case 'o' :
              arg++;
              argcheck(arg, argc, argv);
              p.cp.fout=argv[arg++];
          ...
          case 'u' :                          // mutation rate per bp
            ...
            /* case 'w' :                          // # of generation per year
               arg++;
               argcheck(arg, argc, argv);
               p.mp.g=atof(argv[arg++]);
               if(p.mp.g<=0) messageerr("with -w g option must specify a # of generation per year (e.g.:1/15 for drosophila, 15-20 for Chimp):\n");
               break;
  ...
  /* else if(p.cp.fout[0]!='-')
     {
     if(((p.cp.rfix>0)||(p.cp.uniform[0][1]>0))) messageerr(" -o filename to generate IM input file should not be generated when using recombination.\n");
     else
     if((p.mp.g<=0)) messageerr(" -o filename to generate IM input file require an estimate of the number of years per generations, option -w g.\n");
     }
  ...
  if(p.cp.uniform[0][0]<0)                    // Fixed theta1 value
    {
      ...
      /* Celine changed 11/05/2008 - Uncomment for Celine's use */
      /* if(p.mp.thetafix>.1) messageerrl(" -t theta -> give value for theta (<.1) when theta is fixed. The value is now: theta=", p.mp.thetafix, " \n");
       */////
    }
...
void usage()
{
  fprintf(stderr, "usage: mimarsim Y \n Y=# loci\n");
  ...
  *//////
  ...
  /* Uncommented for Celine's use */
  /* fprintf(stderr, "\t -o filename (filename of the file with data in IM format)\n");
     fprintf(stderr, "\t -w g (g=number of years per generation - Celine - Use for some output)\n");
  *//////
  ...
  *//////
  ...
  fprintf(stderr, "---  Be EXTREMELLY careful with the two following options since they may disrupt the model. I used these options together and in this order to generate structure in the ancestral population.\n");
  fprintf(stderr, "\t -eh t -es t p \n");
  fprintf(stderr, "\t\t -eh (see -ej)\n");
  fprintf(stderr, "\t\t -es (Split population 1 into pop1 and pop2)\n");
  fprintf(stderr, "\t\t  [p is probability that each lineage stays in pop1; p, 1-p are admixture proportions]\n");
  /* fprintf(stderr, "\t\t Size of pop npop is set to N0 and alpha=0.0, size and alpha of pop i are unchanged.\n");
     fprintf(stderr, "\t  (see ej)\n");*/
  /* Uncommented for Celine's use */
